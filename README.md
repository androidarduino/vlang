...
### 高优先级（重要功能）
- ⚪ 函数指针支持（基础函数调用已支持，变量声明待完善）
- ❌ 标准库函数实现（当前通过链接系统 libc）
...
- ❌ 宏高级特性
  ...
  - 连接 `##` 运算符
  - `__VA_ARGS__` 可变参数宏
- ⚪ 浮点数运算（基础运算已支持，优化进行中） ✨ **新增！**
- ❌ 复杂的指针运算优化

### 低优先级（高级功能）
- ❌ `goto` 语句和标签
- ❌ 位字段（bit fields）
- ❌ `volatile` 和 `const` 限定符的完整语义
- ❌ 函数指针数组
- ❌ 变长数组（VLA）
- ❌ `_Generic` 泛型选择
- ❌ 内联函数 `inline`
